...
def main_train(args):
    ...
    voc_size = len(voc)
    if args.resume:  # Comment this when working with the article's pretrained network
    ...
    # Save summary
    os.makedirs(args.train_checkpoints, exist_ok=True)
    tf.summary.scalar(name='train_loss', tensor=train_loss)
    tf.summary.scalar(name='val_loss', tensor=train_loss)
    ...
    model_save_path = os.path.join(args.train_checkpoints, model_name)
    ...
    summary_writer = tf.summary.FileWriter(args.train_checkpoints)
    ...
    with tf.Session(config=config) as sess:
        ...
        if args.resume == True and args.pretrained != '':
            ...
            print('Restore backbone from {}'.format(restore_model_path))
        ...
        while start_iter < args.iters:
            ...
            if start_iter % args.summary_iter == 0:
                ...
                merge_summary_value, val_loss_value, val_pred_value  = sess.run([merge_summary_op, train_loss, train_pred], feed_dict={
                                                                                                         input_train_images: val_data[0],
                                                                                                         input_train_labels: val_data[1],
                                                                                                         input_train_labels_mask: val_data[2],
                                                                                                         input_train_images_width: val_data[4]})
                ...
                if start_iter % args.eval_iter == 0:
                    ...
                    for result, gt in zip(idx2label(train_pred_value, args.voc_type), train_data[3]):
                        print("{} \t {}".format(result, gt))
                    print("#" * 80)
                    print("test prediction \t test labels ")
                    for result, gt in zip(idx2label(val_pred_value, args.voc_type), val_data[3]):
                        print("{} \t {}".format(result, gt))
                    print("#" * 80)

                    train_metrics_result = calc_metrics(idx2label(train_pred_value, args.voc_type), train_data[3], metrics_type="accuracy")
                    val_metrics_result = calc_metrics(idx2label(val_pred_value, args.voc_type), val_data[3], metrics_type="accuracy")
